...
var description = response[i][4];

$$( '#result' ).append('<li class="swipeout"><div class="swipeout-content"><a href="#" onclick="getDeal(\''+ response[i][2]  +'\',\''+ response[i][16]  +'\',\''+ response[i][3]  +'\',\''+ encodeURIComponent(description) +'\')" class="item-content item-link link"><div class="item-media"><img src="http://graph.facebook.com/'+response[i][1]+'/picture?width=50&height=50" style="border-radius:50%;"/></div><div class="item-inner"><div class="item-title-row"><div class="item-title">'+ response[i][3] + '</div></div><div class="item-subtitle">'+ response[i][16] + '</div><div class="item-text">'+ response[i][4] +'</div></div></a></div><div class="swipeout-actions-left"><a href="#" class="bg-blue swipeout-delete swipeout-overswipe" onclick="likeButton(\''+ response[i][2]  +'\')"><i class="pe-7s-like2 pe-2x"></i></a></div><div class="swipeout-actions-right"><a href="#" onclick="closeButton()" class="swipeout-delete swipeout-overswipe"><i class="pe-7s-like2 pe-2x pe-rotate-180"></i></a></div></li>');
...
function getDeal(post_id,name,title,description) {
	

alert (decodeURIComponent(description));

mainView.loadPage('deal.html');
document.getElementById("dealnavbar").innerHTML = title;
document.getElementById("dealresult").innerHTML = title + ',' + description + ',' + name;
